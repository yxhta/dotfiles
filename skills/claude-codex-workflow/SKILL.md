...
- Work inside a git repository. Before delegating, save `git status --short`, full `git diff`, and
  full `git diff --cached` to scratchpad files for attribution. Prefer a clean tree; if pre-existing uncommitted changes exist,
  ask the user to commit/stash, and if declined use degraded mode: mixed-diff reviews,
  baseline-diff attribution, and final-report disclosure.
...
If Codex edits files outside scope, stop, report the affected paths, and ask the user how to proceed.
...
7. Repeat implement, verify, review until the review is clean, budget is exhausted, or user input is
   needed. If Codex reports `blocked` or asks a question, answer unambiguous cases via `--resume`;
   otherwise surface it to the user.
...
Adversarial findings skew speculative: judge each one, discard wrong/out-of-scope ones with a final-report note,
and send Codex only concrete required fixes. Skip only on an explicit quick-pass request, and report it. 9. **Final Claude review**: once the adversarial gate is clean for the current diff
...
as unresolved. 10. Finish with the final report (format below). Never present the result as clean while known findings remain.
